...
<style>
    ...
    div[data-testid="metric-container"] { background-color: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 16px; }
...
Liquidity Ratio: {indicators['liq_ratio']}x
Day Change: {indicators['day_chg']:+.2f}%

Use your web search tool to find recent news, broker commentary, substantial holder notices (Form 603/604), and sector tailwinds/headwinds for {ticker}.

Respond ONLY with a valid JSON object — no markdown fences:
{{
  "signal": "{signal_label}",
  "rationale": "2-3 sentence technical rationale",
  "sentiment": "Current market sentiment, recent news, broker views",
  "holders": "Top holder activity or note if data is limited",
  "risks": "1-2 key risks",
...
Find:
1. Recent ASX announcements and quarterly reports
2. Upcoming scheduled events (AGMs, drilling results, resource estimates, capital raises)
3. Analyst coverage, broker notes, or sentiment shifts
4. Macro or sector events that could materially impact these stocks
...
with st.sidebar:
    ...
    for i, ticker in enumerate(list(st.session_state.form_holdings.keys())):
    ...
    if st.button("✅ Apply Changes", use_container_width=True, type="primary"):
        ...
        st.query_params["portfolio"] = encode_holdings(st.session_state.holdings)
    ...
    share_url = f"https://nhe5bwk4jecpb5xjrkgnia.streamlit.app/?portfolio={encode_holdings(st.session_state.holdings)}"
...
if view == "Portfolio Overview":
    ...
    for ticker, holding in st.session_state.holdings.items():
        ...
        with st.expander(f"🤖 AI Recommendation · Market Sentiment · Holder Activity — {ticker}"):
            ...
            if not ai_unlocked:
                st.warning("🔒 Enter admin password in the sidebar to generate AI analysis.")
            elif st.button(f"Generate Analysis for {ticker}", key=f"btn_{ticker}"):
                with st.spinner(f"Searching market data and analysing {ticker}…"):
                    rec, err = get_ai_recommendation(ticker, name, price, sig_label, sig_score, indicators)
                if err:
                    st.error(err)
                elif rec:
                    ...
                    with col_a:
                        st.markdown("<div class='rec-section-title' style='color:#58a6ff;'>📊 Technical Rationale</div>", unsafe_allow_html=True)
                        st.markdown(rec.get("rationale", ""))
                        st.markdown("<div class='rec-section-title' style='color:#e3b341;margin-top:14px;'>🌐 Market Sentiment</div>", unsafe_allow_html=True)
                        st.markdown(rec.get("sentiment", ""))
                    with col_b:
                        st.markdown("<div class='rec-section-title' style='color:#bc8cff;'>🏦 Top Holder Activity</div>", unsafe_allow_html=True)
                        st.markdown(rec.get("holders", ""))
                        st.markdown("<div class='rec-section-title' style='color:#f85149;margin-top:14px;'>⚠️ Key Risks</div>", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    for ma, color, lbl in [(ma20, "#58a6ff", "MA20"), (ma50, "#e3b341", "MA50"), (ma200, "#bc8cff", "MA200")]:
        fig.add_trace(go.Scatter(x=hist.index, y=ma, line=dict(color=color, width=1), name=lbl), row=1, col=1)
    fig.add_trace(go.Scatter(x=hist.index, y=vwap, line=dict(color="#ff7b72", width=1, dash="dot"), name="VWAP"), row=1, col=1)
    bar_colors = ["#3fb950" if c >= o else "#f85149" for c, o in zip(hist["Close"], hist["Open"])]
    fig.add_trace(go.Bar(x=hist.index, y=hist["Volume"], marker_color=bar_colors, name="Volume", opacity=0.6), row=2, col=1)
...
# ─────────────────────────────────────────────
    ...
    if normed.empty:
        ...
        st.stop()
    colors = {"AKN.AX": "#58a6ff", "XST.AX": "#3fb950", "G11.AX": "#e3b341",
              "VRC.AX": "#bc8cff", "RNX.AX": "#ff7b72"}
    fig = go.Figure()
...
# ─────────────────────────────────────────────
    ...
    if not ai_unlocked:
        st.warning("🔒 Enter admin password in the sidebar to generate the catalyst pipeline.")
    elif st.button("🔍 Generate / Refresh Catalyst Pipeline", type="primary"):
